...
namespace Hazel.Core.Caching
{
    ...
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        /// </summary>
        /// <param name="provider">The provider<see cref="IEasyCachingProvider"/>.</param>
        ...
        public TEntity Get<TEntity>(string key, Func<TEntity> acquire, int? cacheTime = null)
        {
            if (cacheTime <= 0)
                return acquire();

            return _provider.Get(key, acquire, TimeSpan.FromMinutes(cacheTime ?? HazelCachingDefaults.CacheTime))
                .Value;
        }
        ...
        public async Task<TEntity> GetAsync<TEntity>(string key, Func<Task<TEntity>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? HazelCachingDefaults.CacheTime));
            return t.Value;
        }
