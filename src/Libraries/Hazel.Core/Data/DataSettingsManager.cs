...
namespace Hazel.Core.Data
{
    ...
    {
        /// <summary>
        /// Defines the _databaseIsInstalled.
        /// </summary>
        ...
        public static DataSettings LoadSettings(string filePath = null, bool reloadSettings = false, IHazelFileProvider fileProvider = null)
        {
            if (!reloadSettings && Singleton<DataSettings>.Instance != null)
                return Singleton<DataSettings>.Instance;

            fileProvider = fileProvider ?? CommonHelper.DefaultFileProvider;
            filePath = filePath ?? fileProvider.MapPath(HazelDataSettingsDefaults.FilePath);

            //check whether file exists
            if (!fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous hazel versions
                filePath = fileProvider.MapPath(HazelDataSettingsDefaults.ObsoleteFilePath);
                if (!fileProvider.FileExists(filePath))
        ...
        /// <param name="fileProvider">File provider.</param>
        public static void SaveSettings(DataSettings settings, IHazelFileProvider fileProvider = null)
        {
            Singleton<DataSettings>.Instance = settings ?? throw new ArgumentNullException(nameof(settings));

            fileProvider = fileProvider ?? CommonHelper.DefaultFileProvider;
            var filePath = fileProvider.MapPath(HazelDataSettingsDefaults.FilePath);
